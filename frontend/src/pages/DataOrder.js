...
const styles = `
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .input-search { background: var(--pastel-yellow-light); border: 1.5px solid var(--pastel-yellow-medium); border-radius: 50px; padding: 0.55rem 1rem 0.55rem 2.3rem; font-family: 'Poppins', sans-serif; font-size: 0.85rem; color: var(--text-primary); width: 200px; transition: var(--transition); }
    .input-search:focus { outline: none; border-color: var(--pastel-yellow-dark); background: var(--white-soft); width: 240px; }
    .select-filter { background: var(--pastel-yellow-light); border: 1.5px solid var(--pastel-yellow-medium); border-radius: 50px; padding: 0.55rem 1rem; font-family: 'Poppins', sans-serif; font-size: 0.85rem; color: var(--text-primary); appearance: none; }
    ...
    @media (max-width: 767px) { .page-content { padding: var(--spacing-md); } .card-header-custom { padding: var(--spacing-md); flex-direction: column; align-items: flex-start; } .card-actions { width: 100%; } .input-search { width: 100%; } .input-search:focus { width: 100%; } .btn-tambah { width: 100%; justify-content: center; } .col-hide-sm { display: none; } }
`;

// ✅ FUNGSI FORMAT TANGGAL - ROBUST
const formatTanggalDisplay = (dateString) => {
    if (!dateString || dateString === '0000-00-00 00:00:00' || dateString === 'NULL') return '-';
    
    try {
        // Handle berbagai format
        const str = String(dateString);
        
        // Format: "2026-06-16 07:00:00" atau "2026-06-16T07:00:00.000Z"
        ...
        return str; // Return apa adanya jika tidak cocok
    } catch (e) {
...
function DataOrder() {
    ...
    // ✅ State editData - HANYA field yang bisa diedit
    const [editData, setEditData] = useState({ 
        ...
        harga: '', 
        status: 'Diproses',
        tanggal_display: '',  // Hanya untuk tampilan
        estimasi_display: ''  // Hanya untuk tampilan
    });
    ...
    const handleDelete = async (id, kode) => {
        ...
        try {
            ...
            if (result.success) { alert('✅ Order dihapus'); loadOrders(); }
            else alert('❌ ' + (result.message || 'Gagal'));
        } catch (e) { alert('❌ Gagal terhubung ke backend'); }
    };

    // ✅ openEditModal - Tampilkan tanggal untuk dibaca saja
    const openEditModal = (id) => {
        const order = orders.find(o => o.id == id);
        if (!order) return alert('Data tidak ditemukan');
        
        console.log('📦 Order data:', order);
        console.log('📅 Tanggal:', order.tanggal);
        console.log('📅 Estimasi:', order.estimasi_selesai);
        ...
        setEditData({ 
            ...
            // ✅ Format untuk ditampilkan (readonly)
            tanggal_display: formatTanggalDisplay(order.tanggal),
            estimasi_display: formatTanggalDisplay(order.estimasi_selesai)
        });
    ...
    // ✅ handleSaveEdit - TIDAK KIRIM tanggal dan estimasi_selesai!
    const handleSaveEdit = async () => {
        const berat = parseFloat(editData.berat) || 0;
        
        // ✅ HANYA kirim field yang bisa diubah
        const payload = { 
            nama_customer: editData.nama_customer.trim(), 
            no_hp: editData.no_hp.trim(), 
            berat: berat, 
            harga: Math.round(berat * 7000), 
            status: editData.status
            // ❌ TIDAK ADA: tanggal
            // ❌ TIDAK ADA: estimasi_selesai
            // ❌ TIDAK ADA: layanan
            // ❌ TIDAK ADA: catatan
        };
    ...
    const filteredOrders = orders.filter(o => {
        const matchKeyword = o.kode?.toLowerCase().includes(searchKeyword.toLowerCase()) || o.nama_customer?.toLowerCase().includes(searchKeyword.toLowerCase());
        ...
        <>
            ...
            <aside className={`sidebar ${sidebarOpen ? 'open' : ''}`}>
                ...
                <nav className="sidebar-nav">
                    ...
                    <Link to="/data-order" className="nav-item active"><i className="bi bi-list-check"></i><span>Data Order</span><span className="nav-badge">{orders.length}</span></Link>
            ...
            <div className="main-wrapper">
                <header className="topbar">
                    <div className="topbar-left">
                        ...
                        <div className="page-title">Data Order Laundry<span>Kelola semua pesanan masuk</span></div>
                    </div>
                    <div className="topbar-right"><div className="topbar-date">{todayDate}</div></div>
                </header>
                ...
                <main className="page-content">
                    <div className="main-card">
                        <div className="card-header-custom">
                            <div>
                                <div className="card-title"><i className="bi bi-table"></i>Data Order Laundry</div>
                                <div className="card-subtitle">Kelola semua pesanan masuk</div>
                            </div>
                            <div className="card-actions">
                                <div className="search-wrap">
                                    <i className="bi bi-search"></i>
                                    <input type="text" className="input-search" placeholder="Cari nama / kode..." value={searchKeyword} onChange={(e) => setSearchKeyword(e.target.value)} />
                                </div>
                                <select className="select-filter" value={selectedStatus} onChange={(e) => setSelectedStatus(e.target.value)}>
                        ...
                        <div className="table-responsive">
                            <table className="table-order">
                                <thead>
                                    <tr><th>KODE</th><th>NAMA CUSTOMER</th><th>BERAT</th><th className="col-hide-sm">HARGA</th><th className="col-hide-sm">TANGGAL</th><th>STATUS</th><th>AKSI</th></tr>
                                </thead>
                                <tbody>
                                    {loading ? (
                                        ...
                                        <tr className="empty-row"><td colSpan="7"><i className="bi bi-inbox"></i><br/>Tidak ada data yang cocok.</td></tr>
                                    ) : (
                                        filteredOrders.map(order => {
                                            ...
                                            return (
                                                <tr key={order.id}>
                                                    ...
                                                    <td className="date-cell col-hide-sm">{formatTanggalDisplay(order.tanggal)}</td>
                        ...
                        <div className="table-footer">
                            <div className="table-info">{loading ? '-' : `Menampilkan ${filteredOrders.length} dari ${orders.length} order`}</div>
                                ...
                                <div className="modal-body">
                                    <form onSubmit={(e) => e.preventDefault()}>
                                        ...
                                        <div className="mb-3">
                                            ...
                                            <input type="text" value={editData.tanggal_display} className="form-control" readOnly style={{ background: 'var(--pastel-yellow)', cursor: 'not-allowed' }} />
                                            <span className="form-hint">⚠️ Tanggal tidak dapat diubah</span>
                                        </div>

                                        {/* ✅ ESTIMASI - READONLY */}
                                        <div className="mb-3">
                                            <label className="form-label">Estimasi Selesai</label>
                                            <input type="text" value={editData.estimasi_display || '-'} className="form-control" readOnly style={{ background: 'var(--pastel-yellow)', cursor: 'not-allowed' }} />
                                            <span className="form-hint">⚠️ Estimasi tidak dapat diubah</span>
                                        </div>

                                        <div className="mb-3"><label className="form-label">Nama Customer</label><input type="text" name="nama_customer" value={editData.nama_customer} onChange={handleEditChange} className="form-control" required /></div>
                                        ...
                                        <div className="mb-3"><label className="form-label">Berat (kg)</label><input type="number" step="0.1" name="berat" value={editData.berat} onChange={handleEditChange} className="form-control" min="0.1" required /></div>
                                        <div className="mb-3"><label className="form-label">Harga (Rp)</label><input type="number" name="harga" value={editData.harga} className="form-control" readOnly style={{ background: 'var(--pastel-yellow)', cursor: 'not-allowed' }} /></div>
                                        
                                        <div className="mb-3"><label className="form-label">Status</label>
                                            <select name="status" value={editData.status} onChange={handleEditChange} className="form-control" required>
                                                <option value="Diproses">Diproses</option><option value="Dicuci">Dicuci</option>
                                                <option value="Disetrika">Disetrika</option><option value="Selesai">Selesai</option><option value="Diantar">Diantar</option>
                                            </select>
                                        </div>
